...
export async function POST(req: NextRequest) {
  try {
    ...
    await resend.emails.send({
      ...
      subject: `Nouvelle demande CUb — ${nom}`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#0b1220;color:#fff;border-radius:12px;">
          <p style="color:#c29a6b;font-size:11px;text-transform:uppercase;letter-spacing:2px;margin:0 0 16px;">Nouvelle demande dossier CUb</p>
          ...
          <div style="background:#111720;border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:16px;margin-bottom:16px;">
            ...
            <p style="margin:0;"><strong>Parcelle :</strong> ${adresseComplete} — ${parcelle || '—'}</p>
          </div>
          <div style="background:#111720;border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:16px;margin-bottom:16px;">
          ...
          <p style="margin-top:24px;color:#6b7280;font-size:12px;">Paiement en cours — consultez le dashboard admin (onglet CUb).</p>
        </div>
      `,
    }).catch(console.warn)
    ...
    await resend.emails.send({
      ...
      to: email,
      subject: 'Votre demande de dossier CUb a bien été reçue — Closia',
      html: `
        <div style="font-family:Arial,Helvetica,sans-serif;max-width:600px;margin:0 auto;background:#0b1220;color:#fff;border-radius:12px;overflow:hidden;">
          <div style="background:linear-gradient(135deg,#0b1220 0%,#1b2a4a 100%);padding:40px 48px 32px;border-bottom:1px solid rgba(194,154,107,0.3);">
            ...
            <p style="font-size:12px;color:rgba(255,255,255,0.45);text-transform:uppercase;letter-spacing:1.5px;margin:0;">Préparation dossier CUb</p>
          </div>
