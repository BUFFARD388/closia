...
**Ta réponse doit comporter quatre parties distinctes, dans cet ordre exact :**
...
`
    ...
    // Parser les quatre sections
    const screeningMatch = text.match(/---SCREENING---\s*([\s\S]*?)(?=---POTENTIEL_SYNTHETISE---|---BROUILLON_VALIDATION---|$)/)
    const potentielMatch = text.match(/---POTENTIEL_SYNTHETISE---\s*([\s\S]*?)(?=---BROUILLON_VALIDATION---|$)/)
