...
export interface SendRapportEmailParams {
  analyseId: string
  nom: string
  email: string
  adresse: string
  description: string
  rapport: string
  fichiers?: { name: string; url: string }[]
}

// Logique d'envoi du rapport final au client, partagée entre :
// - la route /api/emails/envoyer-rapport (envoi manuel "sans paiement" depuis l'admin)
// - le webhook Stripe (livraison automatique après paiement, voir stripe/webhook/route.ts)
// Appelée directement en process (pas de fetch HTTP interne) pour éviter toute dépendance
// fragile à NEXT_PUBLIC_APP_URL depuis une fonction serverless.
export async function sendRapportEmail(params: SendRapportEmailParams) {
  const { analyseId, nom, email, adresse, description, rapport, fichiers } = params
  const rapportUrl = `https://closia.net/rapport/${analyseId}`

  const filesHtml = fichiers && fichiers.length > 0
